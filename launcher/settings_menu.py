...
from .constants import (
    BG_COLOR, TEXT_COLOR, ACCENT_COLOR, PRIMARY_COLOR, BUTTON_COLOR, BUTTON_HOVER_COLOR, BUTTON_TEXT_COLOR, WHITE, BLACK
)
...
def show_settings_menu(games, screen, clock, config_path, fonts):
    ...
    back_button = Button(real_screen_width//2 - 150, real_screen_height - 120, 300, 60, "Back", color=BUTTON_COLOR, hover_color=BUTTON_HOVER_COLOR, text_color=BUTTON_TEXT_COLOR, font=BODY_FONT)
    ...
    total_width = ip_label.get_width() + ip_box_width + 40 + port_label.get_width() + port_box_width
    ...
    while running:
        ...
        screen.blit(tetris_label, (real_screen_width//2 - tetris_label.get_width()//2, y))
        y += 40
        # Draw IP label and box
        screen.blit(ip_label, (ip_label_x, input_y + (box_height - ip_label.get_height())//2))
        pygame.draw.rect(screen, WHITE, ip_box, 2, border_radius=8)
        ip_surf = font.render(host_ip, True, BLACK)
        screen.blit(ip_surf, (ip_box.x + 10, ip_box.y + (box_height - ip_surf.get_height())//2))
        # Draw Port label and box
        screen.blit(port_label, (port_label_x, input_y + (box_height - port_label.get_height())//2))
        pygame.draw.rect(screen, WHITE, port_box, 2, border_radius=8)
        port_surf = font.render(host_port, True, BLACK)
        screen.blit(port_surf, (port_box.x + 10, port_box.y + (box_height - port_surf.get_height())//2))
        instr = SCORE_FONT.render("Set IP/Port for LAN play. Host: share your IP/port. Join: enter host's IP/port.", True, TEXT_COLOR)
        screen.blit(instr, (real_screen_width//2 - instr.get_width()//2, 430))
        y = 530
        for game in games:
            if game.name != "Tetris Math":
                game_label = font.render(f"{game.name} Settings", True, ACCENT_COLOR)
                screen.blit(game_label, (real_screen_width//2 - game_label.get_width()//2, y))
                y += 40
                game_setting = font.render("(No settings yet)", True, TEXT_COLOR)
                screen.blit(game_setting, (real_screen_width//2 - game_setting.get_width()//2, y))
                y += 60
